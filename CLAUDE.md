...
Figures are **generated by matplotlib inside the notebook**, never linked or embedded as
image files. That keeps the offline rule intact, keeps them regenerable from
`build_notebook.py`, and avoids any licensing question about borrowed images. Palette is
`GREEN = "#2a7"`, `BLUE = "#47c"`, `GREY = "#999"`, `ORANGE = "#e0a300"`, defined in the
first section 0 figure cell and reused after that. Hide top and right spines.
